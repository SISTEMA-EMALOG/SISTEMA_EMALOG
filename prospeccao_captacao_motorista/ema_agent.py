...
from infraestrutura_critica.models import Driver, DriverBid, EmaSession, WhatsAppMessage
...
def _find_session_by_phone(phone: str) -> EmaSession | None:
    ...
    from infraestrutura_critica.utils.evolution_api import normalize_phone
...
def webhook_url():
    """Fonte única da URL de webhook — sempre inclui o token de autenticação.
    Usada tanto pelo painel EMA quanto pela Central de Contratação, para que
    os dois nunca mais registrem versões incompatíveis na Evolution API."""
    from flask import url_for
    return url_for('ema.webhook', _external=True, token=_webhook_token())
...
def start_sessions():
    ...
    from prospeccao_captacao_motorista.utils.ema_agent import (greeting_message, FIELD_DEFS,
...
def configure_webhook():
    ...
    from infraestrutura_critica.utils.evolution_api import set_webhook, get_webhook_info
    ...
    if result.get('ok'):
        ...
        return jsonify({'ok': True, 'webhook_url': url, 'info': info.get('data')})
...
def check_webhook():
    ...
    if not _staff_only():
        return jsonify({'ok': False, 'error': 'Acesso negado'}), 403
    from infraestrutura_critica.utils.evolution_api import get_webhook_info
...
def _process_single_message(msg_data: dict):
    ...
    from oferta_frete_motorista.driver_bids import find_active_bid_by_phone
    ...
    if ema_session:
        ...
        from prospeccao_captacao_motorista.utils.ema_agent import process_message, _append_history
        from infraestrutura_critica.utils.evolution_api import send_text
    ...
    try:
        bid = active_bid
        if bid:
            from oferta_frete_motorista.utils.driver_bid_agent import process_bid_response, apply_bid_result
            from infraestrutura_critica.utils.evolution_api import send_text
...
def _download_media_temp(media_url: str, msg_type: str) -> str | None:
    ...
    try:
        ...
        from infraestrutura_critica.utils.evolution_api import download_media
...
def _download_media_decrypted(msg_data: dict, msg_type: str) -> str | None:
    ...
    """
    from infraestrutura_critica.utils.evolution_api import download_media_from_message
...
def session_detail(sid):
    ...
    from prospeccao_captacao_motorista.utils.ema_agent import driver_missing_fields, driver_completion_pct, FIELD_DEFS
...
def session_send(sid):
    ...
    from prospeccao_captacao_motorista.utils.ema_agent import _append_history
    from infraestrutura_critica.utils.evolution_api import send_text
...
def session_simulate(sid):
    ...
    driver = ema_session.driver
    from prospeccao_captacao_motorista.utils.ema_agent import process_message
    from infraestrutura_critica.utils.evolution_api import send_text
...
def session_reset(sid):
    ...
    ema_session = EmaSession.query.get_or_404(sid)
    driver = ema_session.driver
    from prospeccao_captacao_motorista.utils.ema_agent import FIELD_DEFS, _is_field_applicable, driver_missing_fields
...
def session_restart(sid):
    ...
    from prospeccao_captacao_motorista.utils.ema_agent import (greeting_message, FIELD_DEFS, _is_field_applicable,
...
@login_required
def api_status():
    from infraestrutura_critica.utils.evolution_api import get_connection_status
...
def api_reconnect():
    ...
    if not _staff_only():
        return jsonify({'error': 'Acesso negado'}), 403
    from infraestrutura_critica.utils.evolution_api import restart_instance
...
def api_qrcode():
    ...
    if not _staff_only():
        return jsonify({'error': 'Acesso negado'}), 403
    from infraestrutura_critica.utils.evolution_api import get_qr_code
...
def api_diagnostics(phone):
    ...
    if not _staff_only():
        return jsonify({'error': 'Acesso negado'}), 403
    from infraestrutura_critica.utils.evolution_api import (check_number_on_whatsapp, get_recent_messages,
...
def api_driver_pct(did):
    ...
    from prospeccao_captacao_motorista.utils.ema_agent import driver_completion_pct, driver_missing_fields
