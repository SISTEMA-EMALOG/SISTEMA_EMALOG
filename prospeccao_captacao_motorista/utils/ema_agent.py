...
Conversação via Groq (Llama 3.3 70B). Extração automática de dados por foto de
documento (OCR) está temporariamente desativada — ver _extract_from_image().
    ...
    if first_missing_key == 'cpf':
        ...
        if len(cpf_candidate) == 11:
            if validate_cpf(cpf_candidate):
                ...
                from infraestrutura_critica.models import Driver as _DrCheck
...
def _process_manual_collect(session, driver, text: str, manual_queue: list) -> str:
    ...
    if kind == 'cpf':
        ...
        if len(cleaned) == 11 and validate_cpf(cleaned):
            ...
            from infraestrutura_critica.models import Driver as _DrM
...
def _extract_from_image(image_path: str, doc_key: str) -> dict:
    ...
    OCR de documento DESATIVADO — dependia do proxy de IA do Replit
    (AI_INTEGRATIONS_GEMINI_BASE_URL/API_KEY), que não existe no AWS.
    Reativar exige uma API key real do Google Gemini (AI Studio ou GCP) e
    trocar esta chamada por uma requisição direta à API do Google.
    Callers já tratam retorno {} como "nenhum dado extraído automaticamente"
    — reativar aqui não exige mudança em quem chama esta função.
    """
    logger.info(f"[EMA OCR] Extração automática desativada (sem API key configurada) — doc_key={doc_key}")
    return {}
        ...
        try:
            if kind == 'cpf':
                ...
                if len(cleaned) == 11 and validate_cpf(cleaned):
                    ...
                    from infraestrutura_critica.models import Driver as _Driver
