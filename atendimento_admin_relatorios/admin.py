...
from infraestrutura_critica.models import User, Client, Driver, Quote, Freight, Payment, AuditLog
...
def backup_index():
    ...
    from infraestrutura_critica.utils.backup import list_backups, get_backup_stats
...
def backup_download(filename):
    ...
    from infraestrutura_critica.utils.backup import BACKUP_DIR

    # Segurança: apenas arquivos dentro de BACKUP_DIR com nome esperado
    if not filename.startswith('emalog_backup_') or '..' in filename:
        abort(400)

    from infraestrutura_critica.utils.backup import download_backup
...
def backup_api_status():
    ...
    from infraestrutura_critica.utils.backup import get_backup_stats
