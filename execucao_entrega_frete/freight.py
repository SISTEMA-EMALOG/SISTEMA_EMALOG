...
def view(id):
    ...
    from infraestrutura_critica.models import FreightDocument
...
def update_status(id):
    try:
        ...
        if new_status == 'cancelado' and freight.quote_id:
            from infraestrutura_critica.models import Quote as _Quote
        ...
        from cotacao_precificacao_clientes.notifications import notify_freight_status_update
        notify_freight_status_update(freight, old_status, new_status)
        
        # Se finalizado, enviar email para cliente
        if new_status == 'entregue':
            try:
                from infraestrutura_critica.utils.email_service import send_freight_completed_email
