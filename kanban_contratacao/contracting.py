...
from infraestrutura_critica.models import Client, Driver, DriverBid, EmaSession, Freight, WhatsAppMessage, User
from kanban_contratacao.utils.contracting_service import KANBAN_STAGES, move_bid, stage_for_bid
...
def configure_ema_webhook():
    if not _staff_only():
        return _forbidden()
    from prospeccao_captacao_motorista.ema_agent import webhook_url
    from infraestrutura_critica.utils.evolution_api import set_webhook
...
def ema_sessions():
    if not _staff_only():
        return _forbidden()
    from prospeccao_captacao_motorista.utils.ema_agent import driver_completion_pct, FIELD_MAP
